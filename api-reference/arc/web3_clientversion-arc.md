...
# web3\_clientVersion - ARC

This method returns the version string of the client software running the node. Arc's execution layer is based on a Reth fork (`arc-node-execution`); the exact string depends on the build.
...
```json
{
    "jsonrpc": "2.0",
    "id": "getblock.io",
    "result": "arc-node/v0.5.0-stable/linux-amd64/rustc1.79.0"
}
```
{% endcode %}
...
* Capability detection — feature support varies by client version
* Logging and debugging in multi-client setups
...
const result = await client.request({ method: 'web3_clientVersion', params: [] });
console.log(result);
```
{% endtab %}
{% endtabs %}
