...
# transaction\_v1\_broadcast - Avail

Submits a signed transaction for the node to broadcast and keep re-broadcasting, via the new JSON-RPC specification. Returns an operation id used to stop broadcasting with `transaction_v1_stop`. Fire-and-forget; it does not report inclusion (use `transactionWatch_v1_submitAndWatch` for status).
