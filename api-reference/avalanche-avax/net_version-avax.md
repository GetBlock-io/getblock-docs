...
Returns the current network ID as a decimal string. For Avalanche C-Chain mainnet: `"43114"`; for Fuji testnet: `"43113"`. Decimal string, unlike `eth_chainId` which returns hex.
...
| ------ | ------ | ---------------------------------------------------------------------- |
| result | string | Network ID as a decimal string (`43114` mainnet, `43113` Fuji testnet) |

## Use Cases

* Detecting which network the node is on
* Legacy compatibility — older tools use `net_version` over `eth_chainId`
...
const result = await client.request({ method: 'net_version', params: [] });
console.log(result);
```
{% endtab %}
{% endtabs %}
