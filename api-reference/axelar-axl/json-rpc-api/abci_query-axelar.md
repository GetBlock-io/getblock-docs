...
Executes an ABCI query against application state at a path, returning the raw (base64) value. The low-level way to read any Cosmos SDK or Axelar module store; underlies the REST gateway.
...
* **Axelar Modules**: Read deployment/market/provider state
