...
# subscribeAddresses - Bitcoin Cash
...
| --- | --- | --- | --- |
| addresses | array | Yes | Bitcoin Cash addresses to watch, in CashAddr or legacy format |
...
{% hint style="warning" %}
An unconfirmed transaction arrives with **`blockHeight` set to `0`** in this notification, while the REST [api/v2/tx](../bitcoin-cash-blockbook-rest-api/api-v2-tx-bitcoin-cash.md) endpoint reports `-1` for the same state. Detect pending status with `confirmations === 0` so one check works across both interfaces.
