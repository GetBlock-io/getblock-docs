...
| ----------- | ------- | -------- | ---------------------------------------- |
| blockNumber | integer | No       | Block number. Omit for the latest block. |
...
* Converting block number to hash for `chain_getBlock` / `chain_getHeader` queries
* Building chain-tip awareness in indexers
...
// Typed wrapper: api.rpc.chain.getBlockHash(...)
const result = await api.rpc.chain.getBlockHash(5870042);
...
# Generic RPC call:
result = substrate.rpc_request("chain_getBlockHash", [5870042])
print(result)
```
{% endtab %}
{% endtabs %}
