...
| --------------------- | ------ | ------------------------------------------------ |
| result.parentHash     | string | Parent block hash                                |
| result.number         | string | Block number in hex                              |
| result.stateRoot      | string | State trie root hash                             |
| result.extrinsicsRoot | string | Extrinsics trie root hash                        |
| result.digest         | object | Block digest — consensus engine logs, seal, etc. |

## Use Cases

* Lightweight block scanning (faster than fetching full blocks)
* Header-chain verification in light clients
* Real-time block notifications with minimal data transfer
...
// Typed wrapper: api.rpc.chain.getHeader(...)
const result = await api.rpc.chain.getHeader("0xc0a6cfca5d4f9e1cb8a5c4b3d2e7a9c4f1b8e6d5c3a2b1f9e8d7c6b5a4f3e2d1c");
console.log(result.toHuman());

// Or use the raw RPC interface for any method:
// const raw = await api.rpc.send({ method: 'chain_getHeader', params: ["0xc0a6cfca5d4f9e1cb8a5c4b3d2e7a9c4f1b8e6d5c3a2b1f9e8d7c6b5a4f3e2d1c"] });
```
{% endtab %}
...
# Generic RPC call:
result = substrate.rpc_request("chain_getHeader", ["0xc0a6cfca5d4f9e1cb8a5c4b3d2e7a9c4f1b8e6d5c3a2b1f9e8d7c6b5a4f3e2d1c"])
print(result)
```
{% endtab %}
{% endtabs %}
