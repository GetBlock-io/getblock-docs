---
description: >-
  Example code for the chain_subscribeFinalizedHeads JSON-RPC method. Complete
  guide on how to use chain_subscribeFinalizedHeads JSON-RPC in GetBlock Web3
  documentation.
---

# chain\_subscribeFinalizedHeads - Bittensor
...
| ------ | ------ | ------------------------------------------------ |
| result | string | Subscription ID for finalized head notifications |
...
// Typed wrapper: api.rpc.chain.subscribeFinalizedHeads(...)
const result = await api.rpc.chain.subscribeFinalizedHeads();
console.log(result.toHuman());

// Or use the raw RPC interface for any method:
// const raw = await api.rpc.send({ method: 'chain_subscribeFinalizedHeads', params: [] });
```
{% endtab %}
...
# Generic RPC call:
result = substrate.rpc_request("chain_subscribeFinalizedHeads", [])
print(result)
```
{% endtab %}
{% endtabs %}
