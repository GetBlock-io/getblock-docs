...
| --------- | --------------- | -------- | -------------------- |
| address   | DATA (20 bytes) | Yes      | EVM address to check |
...
| ------ | -------- | ------------------------------------------------------------ |
| result | QUANTITY | Balance in wei (hex). Divide by 10^18 to get TAO on EVM side |

## Use Cases

* EVM wallet balance display
* Pre-flight funds checks before EVM transactions
* Smart contract reads of native TAO balances
...
const result = await provider.send('eth_getBalance', ["0x7a3F8b9c2d1e4a6f8c5b3d2e7a9c4f1b8e6d5c3a", "latest"]);
console.log(result);
...
const result = await client.request({
    method: 'eth_getBalance',
    params: ["0x7a3F8b9c2d1e4a6f8c5b3d2e7a9c4f1b8e6d5c3a", "latest"]
});
console.log(result);
```
{% endtab %}
