...
| --------- | ------ | -------- | ----------------------------------------------------------- |
| blockHash | string | No       | Block hash to query metadata at. Omit for the latest block. |
...
```json
{
    "jsonrpc": "2.0",
    "id": "getblock.io",
    "result": "0x6d6574610e441853797374656d3853797374656d436f6e6669675c..."
}
```
{% endcode %}
...
| ------ | ------ | ---------------------------------------------------------------------------------- |
| result | string | Hex-encoded SCALE-encoded runtime metadata (typically very large — multi-kilobyte) |

## Use Cases

* Client initialization — Polkadot.js and other clients call this on connect to learn the runtime layout
* Detecting runtime upgrades (metadata changes between releases)
...
// Typed wrapper: api.rpc.state.getMetadata(...)
const result = await api.rpc.state.getMetadata();
console.log(result.toHuman());

// Or use the raw RPC interface for any method:
// const raw = await api.rpc.send({ method: 'state_getMetadata', params: [] });
```
{% endtab %}
...
# Generic RPC call:
result = substrate.rpc_request("state_getMetadata", [])
print(result)
```
{% endtab %}
{% endtabs %}
