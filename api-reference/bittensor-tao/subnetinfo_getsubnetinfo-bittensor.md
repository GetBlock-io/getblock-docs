...
| --------- | ------- | -------- | -------------------------------------------------- |
| netuid    | integer | Yes      | Subnet ID (numeric, e.g. `1` for the root subnet)  |
...
| ------ | -------------- | -------------------------------------------------------------- |
| result | object \| null | Subnet info, or `null` if no subnet exists at the given netUid |
...
result = subtensor.substrate.rpc_request("subnetInfo_getSubnetInfo", [1])
print(result)
```
{% endtab %}
