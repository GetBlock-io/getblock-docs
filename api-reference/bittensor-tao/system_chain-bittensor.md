...
| ------ | ------ | ----------------------------------- |
| result | string | Chain name — `Bittensor` on mainnet |

### Use Cases

* Initial sanity check after connecting to a new endpoint
* Multi-chain dashboards that need to label the connected network
* Distinguishing mainnet from testnet (testnet returns a different chain name)
...
// Typed wrapper: api.rpc.system.chain(...)
const result = await api.rpc.system.chain();
console.log(result.toHuman());

// Or use the raw RPC interface for any method:
// const raw = await api.rpc.send({ method: 'system_chain', params: [] });
```
{% endtab %}
...
# Generic RPC call:
result = substrate.rpc_request("system_chain", [])
print(result)
```
{% endtab %}
{% endtabs %}
