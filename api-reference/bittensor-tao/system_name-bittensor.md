...
| ------ | ------ | ------------------------ |
| result | string | Node implementation name |

## Use Cases

* Identifying which Substrate node implementation backs an endpoint
* Logging for debugging RPC-level issues
...
// Typed wrapper: api.rpc.system.name(...)
const result = await api.rpc.system.name();
console.log(result.toHuman());

// Or use the raw RPC interface for any method:
// const raw = await api.rpc.send({ method: 'system_name', params: [] });
```
{% endtab %}
...
# Generic RPC call:
result = substrate.rpc_request("system_name", [])
print(result)
```
{% endtab %}
{% endtabs %}
