---
description: >-
  GetBlock provides fast and reliable access to Blast nodes via JSON-RPC API.
  Connect to the Blast network without running your own infrastructure.
---

# Blast

Blast is an EVM-compatible Layer 2 built on the OP Stack as an optimistic rollup, distinguished by native yield: bridged ETH and the USDB stablecoin auto-rebase, so holder balances grow over time without any transfer. It settles to Ethereum L1 and targets roughly two-second block times. Because Blast is EVM-equivalent, standard Ethereum contracts, JSON-RPC methods, and tooling such as Foundry, Hardhat, Ethers.js, and Viem work without modification. Native gas is paid in ETH.

### Key Features

* **EVM Equivalence**: Bytecode-level compatibility with Ethereum; contracts and tooling port over unchanged
* **Optimistic Rollup**: Built on the OP Stack, settling to Ethereum L1 with fraud-proof security
* **Native Yield**: Bridged ETH and the USDB stablecoin auto-rebase, accruing yield directly into balances
* **Yield Predeploys**: Blast system contracts live at fixed `0x43...` addresses (Blast yield config, USDB, rebasing WETH)
...
_GetBlock's RPC API reference documentation is provided exclusively for informational purposes and to optimize the developer experience. Blast implements the standard Ethereum JSON-RPC interface; the canonical specification for these methods is the Ethereum JSON-RPC specification at_ [_ethereum.org_](https://ethereum.org/en/developers/docs/apis/json-rpc/)_, and Blast-specific behaviour is documented at_ [_docs.blast.io_](https://docs.blast.io/)_._
{% endhint %}

## Network Information

<table data-search="false"><thead><tr><th>Property</th><th>Value</th></tr></thead><tbody><tr><td>Network Name</td><td>Blast</td></tr><tr><td>Chain ID</td><td>81457</td></tr><tr><td>Native Currency</td><td>ETH (auto-rebasing)</td></tr><tr><td>EVM Compatible</td><td>Yes</td></tr><tr><td>Settlement Layer</td><td>Ethereum L1 (optimistic rollup)</td></tr><tr><td>Block Time</td><td>~2 seconds</td></tr><tr><td>Finality</td><td>L1-backed after the challenge window</td></tr><tr><td>Native Yield</td><td>ETH and USDB auto-rebase</td></tr></tbody></table>
...
```bash
mkdir blast-api-quickstart && cd blast-api-quickstart && npm init --yes
```
{% endstep %}
...
```bash
mkdir blast-api-quickstart && cd blast-api-quickstart
...
* [Blastscan Explorer](https://blastscan.io/)
