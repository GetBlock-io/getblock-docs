...
curl --location --request GET 'https://shared.eu-central-1.getblock.io/<ACCESS-TOKEN>/block?height=23456789'
```
{% endtab %}
...
const config = {
    method: 'post',
    url: 'https://shared.eu-central-1.getblock.io/<ACCESS-TOKEN>/block',
    headers: {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = client
        .post("https://shared.eu-central-1.getblock.io/<ACCESS-TOKEN>/block")
        .header("Content-Type", "application/json")
        .json(&payload)
        .send()
        .await?;
