...
| --------- | ------ | -------- | ----------------------------- |
| query     | string | Yes      | Tendermint event query string |
...
| ------ | ------ | ------------------------------------------------------------------------------- |
| result | object | Empty object on success — streamed events arrive as separate WebSocket messages |
...
| -32603      | Subscription limit exceeded | Each WebSocket connection has a maximum subscription count (typically 5) |
| -32602      | Invalid query               | Query string is malformed or references unknown event types              |
...
const stream = wsClient.listen({"jsonrpc": "2.0", "id": "sub-1", "method": "subscribe", "params": {"query": "tm.event='NewBlock'"}});
stream.subscribe({
...
async def stream():
    async with websockets.connect('wss://go.getblock.io/<ACCESS-TOKEN>/websocket') as ws:
        await ws.send(json.dumps({'jsonrpc': '2.0', 'id': 'sub-1', 'method': 'subscribe', 'params': {"query": "tm.event='NewBlock'"}}))
        async for msg in ws:
