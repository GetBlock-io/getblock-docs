...
curl --location --request POST 'https://shared.eu-central-1.getblock.io/<ACCESS-TOKEN>/unconfirmed_txs' \
...
curl --location --request GET 'https://shared.eu-central-1.getblock.io/<ACCESS-TOKEN>/unconfirmed_txs?limit=30'
```
{% endtab %}
...
const config = {
    method: 'post',
    url: 'https://shared.eu-central-1.getblock.io/<ACCESS-TOKEN>/unconfirmed_txs',
    headers: {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = client
        .post("https://shared.eu-central-1.getblock.io/<ACCESS-TOKEN>/unconfirmed_txs")
        .header("Content-Type", "application/json")
        .json(&payload)
        .send()
        .await?;
