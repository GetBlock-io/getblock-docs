---
description: >-
  GetBlock provides fast and reliable access to Dash nodes via JSON-RPC API.
  Connect to the Dash network without running your own infrastructure.
---

# Dash JSON-RPC API

The Dash Core JSON-RPC interface: Bitcoin Core-style methods for blocks, transactions, the mempool, and network state, plus Dash-specific methods for masternodes and LLMQ quorums. All calls are JSON-RPC 2.0 POST requests to the endpoint base URL.
...
GetBlock provides access to Dash Node JSON-RPC methods. Methods that require wallet or node-administration privileges are not available on shared endpoints and are omitted below.
