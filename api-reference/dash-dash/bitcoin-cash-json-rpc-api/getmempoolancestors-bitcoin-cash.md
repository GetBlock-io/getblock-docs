...
description: >-
  ...
  how to use the getmempoolancestors JSON-RPC method in the GetBlock Web3
  documentation.
---

# getmempoolancestors - Bitcoin Cash

This method returns the in-mempool ancestors of a transaction: the unconfirmed transactions it depends on. Verbose mode returns full entry data for each ancestor.
...
const { data } = await axios.post('https://shared.eu-central-1.getblock.io/<ACCESS-TOKEN>/', {
  jsonrpc: '2.0', method: 'getmempoolancestors',
...
| result    | array\|object | Array of ancestor txids, or detailed objects when verbose is true |

## Use Cases

* **Dependency Resolution**: Find unconfirmed parents that must confirm first
* **CPFP Planning**: Identify the ancestor chain when fee-bumping a transaction
* **Package Relay**: Assemble a transaction package with all its dependencies
* **Spend Ordering**: Order dependent transactions correctly before broadcast
