---
description: >-
  Example code for the getmempoolentry JSON-RPC method. Complete guide on how to
  use getmempoolentry JSON-RPC in GetBlock Web3 documentation.
---

# getmempoolentry - Litecoin

Returns mempool data for given transaction.

### Parameters

| Parameter | Type   | Description                              |
| --------- | ------ | ---------------------------------------- |
| txid      | string | The transaction id (must be in mempool). |

### Request

{% tabs %}
{% tab title="cURL" %}
```bash
curl --location --request POST 'https://go.getblock.io/<ACCESS-TOKEN>/' \
--header 'Content-Type: application/json' \
--data-raw '{
    "jsonrpc": "2.0",
    "method": "getmempoolentry",
    "params": ["txid123..."],
    "id": "getblock.io"
}'
```
{% endtab %}

{% tab title="Axios (JavaScript)" %}
{% code title="example.js" %}
```javascript
import axios from 'axios';

const data = JSON.stringify({
    "jsonrpc": "2.0",
    "method": "getmempoolentry",
    "params": ["txid123..."],
    "id": "getblock.io"
});

const config = {
    method: 'post',
    url: 'https://go.getblock.io/<ACCESS-TOKEN>/',
    headers: {
        'Content-Type': 'application/json'
    },
    data: data
};

axios(config)
    .then(response => console.log(JSON.stringify(response.data)))
    .catch(error => console.log(error));
```
{% endcode %}
{% endtab %}

{% tab title="Python (requests)" %}
{% code title="example.py" %}
```python
import requests
import json

url = "https://go.getblock.io/<ACCESS-TOKEN>/"

payload = json.dumps({
    "jsonrpc": "2.0",
    "method": "getmempoolentry",
    "params": ["txid123..."],
    "id": "getblock.io"
})

headers = {
    'Content-Type': 'application/json'
}

response = requests.post(url, headers=headers, data=payload)
print(response.text)
```
{% endcode %}
{% endtab %}

{% tab title="Rust (reqwest)" %}
{% code title="main.rs" %}
```rust
use reqwest::header;

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let client = reqwest::Client::new();
    
    let response = client
        .post("https://go.getblock.io/<ACCESS-TOKEN>/")
        .header(header::CONTENT_TYPE, "application/json")
        .body(r#"{
            "jsonrpc": "2.0",
            "method": "getmempoolentry",
            "params": ["txid123..."],
            "id": "getblock.io"
        }"#)
        .send()
        .await?;
    
    println!("{}", response.text().await?);
    Ok(())
}
```
{% endcode %}
{% endtab %}
{% endtabs %}

### Response

```json
{
    "jsonrpc": "2.0",
    "id": "getblock.io",
    "result": {
        "vsize": 144,
        "weight": 573,
        "fee": 0.00001000,
        "modifiedfee": 0.00001000,
        "time": 1640000000,
        "height": 2750000,
        "descendantcount": 1,
        "descendantsize": 144,
        "descendantfees": 1000,
        "ancestorcount": 1,
        "ancestorsize": 144,
        "ancestorfees": 1000,
        "depends": []
    }
}
```

### Response Parameters

| Field   | Type   | Description                              |
| ------- | ------ | ---------------------------------------- |
| vsize   | number | Virtual transaction size.                |
| fee     | number | Transaction fee in LTC.                  |
| time    | number | Local time transaction entered pool.     |
| depends | array  | Unconfirmed transactions used as inputs. |

### Use Case

The `getmempoolentry` method is essential for:

* Transaction monitoring
* Fee analysis
* Mempool inspection
* CPFP analysis

### Error Handling

| Status Code | Error Message | Cause                            |
| ----------- | ------------- | -------------------------------- |
| 403         | Forbidden     | Missing or invalid ACCESS-TOKEN. |

### Integration Notes

The `getmempoolentry` method helps developers build robust applications that interact with the Litecoin blockchain.
