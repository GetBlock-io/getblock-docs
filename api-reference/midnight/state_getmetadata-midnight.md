...
| --------- | ------ | -------- | ------------------------------------------------------------------- |
| hash      | string | No       | Block hash to query metadata at. Defaults to the current best block |
...
| ------ | ------ | ------------------------------------- |
| result | string | Hex-encoded SCALE-serialized metadata |
...
// const raw = await (api as any)._rpcCore.provider.send('state_getMetadata', []);
```
{% endtab %}
