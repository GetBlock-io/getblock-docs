...
| -------------------- | ------- | ---------------------------------------------- |
| result.startingBlock | integer | Block number at which the node started syncing |
| result.currentBlock  | integer | Block number the node has synced up to         |
| result.highestBlock  | integer | Highest block known to the network             |
...
// const raw = await (api as any)._rpcCore.provider.send('system_syncState', []);
```
{% endtab %}
...
# Generic RPC call:
result = substrate.rpc_request('system_syncState', [])
print(result)
```
{% endtab %}
{% endtabs %}
