...
| --------- | ------ | -------- | ----------------------- |
| hash      | string | Yes      | The block's sha256 hash |
...
| ------------------------------ | ------------ | -------------------------------------- |
| result.block\_header.hash      | string       | Block hash (echoed back)               |
| result.block\_header.height    | unsigned int | Block height                           |
| result.block\_header.depth     | unsigned int | Number of blocks succeeding this block |
...
* Resolving header data from a known block hash
* Indexers that need only block metadata
* Reorg detection by comparing hashes at the same height
...
const result = await daemon.getDaemonConnection().sendJsonRequest('get_block_header_by_hash', {"hash": "e22cf75f39ae720e8b71b3d120a5ac03f0db50bba6379e2850975b4859190bc6"});
console.log(result);
```
{% endtab %}
...
# For raw RPC access:
result = backend.raw_request('get_block_header_by_hash', {"hash": "e22cf75f39ae720e8b71b3d120a5ac03f0db50bba6379e2850975b4859190bc6"})
print(result)
```
{% endtab %}
{% endtabs %}
