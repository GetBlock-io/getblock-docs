...
| ---------- | ------------ | -------- | ----------------------- |
| params\[0] | unsigned int | Yes      | Block height to look up |
...
| ------ | ------ | ------------------------------ |
| result | string | Block hash at the given height |
...
const result = await daemon.getDaemonConnection().sendJsonRequest('on_get_block_hash', [912345]);
console.log(result);
```
{% endtab %}
...
# For raw RPC access:
result = backend.raw_request('on_get_block_hash', [912345])
print(result)
```
{% endtab %}
{% endtabs %}
