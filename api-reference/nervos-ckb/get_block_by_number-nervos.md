---
description: >-
  Example code for the get_block_by_number JSON-RPC method. Complete guide on
  how to use get_block_by_number JSON-RPC in GetBlock Web3 documentation.
---

# get\_block\_by\_number - Nervos

Returns the information about a block by block number. Same response schema as `get_block`. Convenient when you have the height but not the hash.
...
| with\_cycles  | boolean | No       | If `true`, include per-transaction cycles             |
...
const result = await rpc.send('get_block_by_number', ["0xe5a45f", "0x2", false]);
console.log(result);
```
{% endtab %}
