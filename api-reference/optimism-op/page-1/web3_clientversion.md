...
{% hint style="success" %}
The **web3\_clientVersion** method returns the version string of the Optimism client software.
{% endhint %}
...
The web3\_clientVersion **RPC Optimism** method supports the following network types:
...
To make a request using the web3\_clientVersion **RPC Optimism** method via JSON-RPC, use the following **curl** command:
...
```json
{
    "jsonrpc": "2.0",
    "id": "getblock.io",
    "result": "op-geth/v1.101315.0-stable/linux-amd64/go1.21"
}
```
...
Here's how to use the web3\_clientVersion method with different programming languages:
...
if response.status_code == 200:
    ...
    print("web3_clientVersion result:", result)
else:
...
axios.post(url, payload, { headers })
    .then(response => {
        if (response.status === 200) {
            console.log("web3_clientVersion result:", response.data.result);
        } else {
