...
# eth\_newblockfilter - opBNB

This method creates a filter that emits the hash of every new block produced on opBNB. Poll it with `eth_getFilterChanges` to receive new block hashes since the previous poll.
...
* Server-side new-block detection without WebSocket
* Triggering downstream indexing jobs on each new block
* Simple heartbeat for monitoring tools
...
const result = await client.request({ method: 'eth_newBlockFilter', params: [] });
console.log(result);
```
{% endtab %}
{% endtabs %}
