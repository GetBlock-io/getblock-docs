...
| --------- | ------ | -------- | ----------------------- |
| filterId  | string | Yes      | The filter ID to remove |
...
| ------ | ------- | -------------------------------------------------------------------- |
| result | boolean | `true` if the filter was uninstalled successfully, `false` otherwise |
...
const result = await provider.send('eth_uninstallFilter', ["0x16f1f5f5dc91c4a1d2e7e8b6c4d3f2e1"]);
console.log(result);
```
{% endcode %}
{% endtab %}
...
const result = await client.request({ method: 'eth_uninstallFilter', params: ["0x16f1f5f5dc91c4a1d2e7e8b6c4d3f2e1"] });
console.log(result);
```
{% endcode %}
{% endtab %}
