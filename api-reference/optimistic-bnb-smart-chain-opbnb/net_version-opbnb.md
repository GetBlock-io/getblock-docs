...
This method returns the current network ID as a decimal string. For opBNB mainnet the value is `"204"`; for testnet, `"5611"`. Note this returns a decimal string, unlike `eth_chainId` which returns hex.
...
* Legacy compatibility — many older tools use `net_version` rather than `eth_chainId`
