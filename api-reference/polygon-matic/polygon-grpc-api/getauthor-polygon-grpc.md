---
description: >-
  Example code for the GetAuthor grpc method. Сomplete guide on how to use
  GetAuthor grpc in GetBlock.io Web3 documentation.
---

# GetAuthor - Polygon gRPC

Returns the address of the validator that authored (produced and signed) a specific block. Bor uses secp256k1 signatures appended to the `extraData` header field to identify block authors — this method recovers the author from the signature. Used by Heimdall to verify that blocks were produced by the correct validator subset for the active span.

## Request Parameters

| Parameter | Type   | Required | Description                                |
| --------- | ------ | -------- | ------------------------------------------ |
| `number`  | string | Yes      | Block reference — tag or hex block number  |
...
client.GetAuthor(request, metadata, (err, response) => {
    if (err) {
...
request = bor_pb2.GetAuthorRequest(
    number="latest",
)
response = stub.GetAuthor(request, metadata=metadata)
...
| -------- | ---- | ------------------------------------ |
| `author` | H160 | The block author (validator) address |

## Use Cases

* Validator dashboards showing block production distribution
* Detecting when blocks are produced by unauthorized validators (a Heimdall-side sanity check)
...
| INVALID\_ARGUMENT (3)   | malformed block reference         | Block reference is not a valid tag or hex number                                 |
...
const response = await new Promise((resolve, reject) => {
    client.GetAuthor(request, metadata, (err, res) => err ? reject(err) : resolve(res));
});
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = tonic::Request::new(GetAuthorRequest { /* fields */ });
    let response = client.get_author(request).await?;
    println!("{:?}", response);
    Ok(())
}
```
