---
description: >-
  Example code for the GetTdByNumber grpc method. Сomplete guide on how to use
  GetTdByNumber grpc in GetBlock.io Web3 documentation.
---

# GetTdByNumber - Polygon gRPC

Returns the total difficulty for a block identified by number (or tag). Same output as `GetTdByHash` but with block-number-based lookup — convenient when iterating over block heights without holding hashes.

## Request Parameters

| Parameter | Type   | Required | Description                                           |
| --------- | ------ | -------- | ----------------------------------------------------- |
| `number`  | string | Yes      | Block reference — tag or hex block number or "latest" |
...
client.GetTdByNumber(request, metadata, (err, response) => {
    if (err) {
...
request = bor_pb2.GetTdByNumberRequest(
    number="latest",
)
response = stub.GetTdByNumber(request, metadata=metadata)
...
func main() {
    ...
    req := &bor.GetTdByNumberRequest{
        Number: "latest",
    }
    resp, err := client.GetTdByNumber(ctx, req)
    if err != nil {
...
* Building a TD-vs-height chart for chain difficulty growth analytics
* Fork-choice implementations that iterate by height
...
const response = await new Promise((resolve, reject) => {
    client.GetTdByNumber(request, metadata, (err, res) => err ? reject(err) : resolve(res));
});
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = tonic::Request::new(GetTdByNumberRequest { /* fields */ });
    let response = client.get_td_by_number(request).await?;
    println!("{:?}", response);
    Ok(())
}
```
