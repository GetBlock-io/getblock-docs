...
# eth\_getTransactionByBlockNumberAndIndex - Ronin

This method returns a transaction by its position in a block identified by number. The index is the transaction's zero-based position within the block.
...
```json
{
    ...
    "result": {
        ...
        "to": "0xe514d9deb7966c8be0ca922de8a064264ea6bcd4",
...
* **Indexing**: Reconstruct block contents transaction by transaction
* **Explorer Detail**: Resolve a block-and-index reference to a transaction
...
const tx = await provider.send('eth_getTransactionByBlockNumberAndIndex', ['0x1e8480', '0x0']);
console.log(tx);
```
{% endcode %}
{% endtab %}
...
const client = createPublicClient({ chain: ronin, transport: http('https://shared.eu-central-1.getblock.io/<ACCESS-TOKEN>/') });

const tx = await client.getTransaction({ blockNumber: 2000000n, index: 0 });
console.log(tx);
```
{% endcode %}
{% endtab %}
