---
description: >-
  Example code for the NameService/LookupName gRPC method. Complete guide on how
  to use NameService/LookupName gRPC method in GetBlock Web3 documentation.
---
...
**Full method path**: `sui.rpc.v2.NameService/LookupName`

## Request Fields

| Field | Type   | Required | Description                             |
| ----- | ------ | -------- | --------------------------------------- |
| name  | string | Yes      | SuiNS name — typically ends with `.sui` |
...
response = stub.LookupName(request, metadata=metadata)
print(response)
```
{% endtab %}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = client.lookup_name(request).await?;
...
```json
{
    ...
    "expiration_timestamp_ms": "1806586511019"
}
```
{% endcode %}
...
| ------------------------- | ------- | --------------------------------------------------- |
| name                      | string  | Echoed name                                         |
| address                   | Address | Resolved 32-byte address                            |
| expiration\_timestamp\_ms | string  | Unix-millis timestamp when the registration expires |
...
| NOT\_FOUND          | 5       | Name is not registered or has expired                                               |
...
// raw gRPC call to sui.rpc.v2.NameService/LookupName is available as shown in the
...
# Consult pysui docs for the wrapper of NameService.LookupName.
```
{% endtab %}
{% endtabs %}
