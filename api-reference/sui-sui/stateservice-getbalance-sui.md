---
description: >-
  Example code for the StateService/GetBalance gRPC method. Complete guide on
  how to use StateService/GetBalance gRPC method in GetBlock Web3 documentation.
---
...
**Full method path**: `sui.rpc.v2.StateService/GetBalance`
...
| coin\_type | string  | Yes      | Move coin type — e.g. `0x2::sui::SUI` for the native token |
...
response = stub.GetBalance(request, metadata=metadata)
print(response)
```
{% endtab %}
...
| --------------------- | ------ | ------------------------------------------------------------------------------------------------------ |
| balance.coin\_type    | string | Echoed coin type                                                                                       |
| balance.balance       | string | Total balance across all coin objects of this type (uint64 in the coin's smallest unit — MIST for SUI) |
| balance.coin\_balance | string | Same value as `balance` — included for backward compatibility                                          |
...
| NOT\_FOUND          | 5       | The address has no coin objects of the requested type                               |
| INVALID\_ARGUMENT   | 3       | Malformed address or coin type                                                      |
...
// raw gRPC call to sui.rpc.v2.StateService/GetBalance is available as shown in the
...
# Consult pysui docs for the wrapper of StateService.GetBalance.
```
{% endtab %}
{% endtabs %}
