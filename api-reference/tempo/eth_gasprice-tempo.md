...
Returns the current gas price in TIP-20 fee units. Tempo's fee model produces predictable, stable fees (\~$0.001 per payment transaction) thanks to dedicated payment lanes that reserve protocol-level blockspace.
...
| ------ | ------ | ----------------------------------------------------- |
| result | string | Current suggested gas price in TIP-20 fee units (hex) |

## Use Cases

* Setting `gasPrice` on legacy transactions
* Computing estimated TIP-20 cost before broadcast
* Fee oracle inputs for payment-routing logic
