---
description: >-
  Example code for the getconfigparam JSON-RPC method. Сomplete guide on how to
  use getconfigparam JSON-RPC in GetBlock.io Web3 documentation.
---
...
| --------- | ------- | -------- | ------------------------------------------------------------------------------------------------ |
| param     | integer | Yes      | Configuration parameter number (e.g. `0` for config address, `20` for gas prices on masterchain) |
| seqno     | integer | No       | Masterchain block seqno at which to read the config. Defaults to the latest                      |
...
```bash
curl --location --request GET 'https://shared.eu-central-1.getblock.io/<ACCESS-TOKEN>/getConfigParam?param=20' \
...
| ------------------- | ------ | ------------------------------------------------- |
| result.config.@type | string | Always `tvm.cell`                                 |
| result.config.bytes | string | Base64-encoded BOC of the configuration parameter |
