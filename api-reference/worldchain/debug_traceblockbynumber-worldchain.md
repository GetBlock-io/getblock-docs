...
# debug\_traceblockbynumber - Worldchain

Returns all traces of all transactions in a block by block number on the World Chain network.
...
The `debug_traceBlockByNumber` method on World Chain is typically used for:
...
const traces = await provider.send('debug_traceBlockByNumber', ['0x1A5C3B2', {}]);
