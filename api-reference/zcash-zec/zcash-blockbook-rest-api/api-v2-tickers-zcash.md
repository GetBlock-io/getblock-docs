...
# api/v2/tickers - Zcash

This endpoint returns current or historical fiat exchange rates for Zcash. A timestamp or block selects historical rates; without either, the latest available rates are returned.
...
{% hint style="info" %}
`ts` is the timestamp of the rate actually returned, which can differ from the one requested. A rate of `-1` marks a currency that is unavailable for that timestamp.
{% endhint %}

## Use Cases

* **Fiat Display**: Show a ZEC amount in a user's local currency
* **Historical Valuation**: Value a past transaction at the rate of its block
* **Invoicing**: Convert a fiat-priced invoice into ZEC at the current rate
