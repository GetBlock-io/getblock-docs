...
| ------ | ------ | -------------------------------------------------------------------------- |
| result | string | Suggested max priority fee per gas in wei (hex); typically `0x0` on zkSync |
...
result = zk_web3.zksync._zks_endpoints if 'eth_maxPriorityFeePerGas'.startswith('zks_') else None
# Or call any method generically:
result = zk_web3.provider.make_request('eth_maxPriorityFeePerGas', [])
print(result)
```
{% endtab %}
{% endtabs %}
