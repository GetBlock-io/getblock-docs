# net\_peercount zksync

Returns the number of peers currently connected to the node, as a hex string.
...
| ------ | ------ | ------------------------------------------- |
| result | string | Number of peers connected to the node (hex) |
...
result = zk_web3.zksync._zks_endpoints if 'net_peerCount'.startswith('zks_') else None
# Or call any method generically:
result = zk_web3.provider.make_request('net_peerCount', [])
print(result)
```
{% endtab %}
{% endtabs %}
