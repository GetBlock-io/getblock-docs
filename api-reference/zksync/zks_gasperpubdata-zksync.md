...
| ------ | ------ | ---------------------------------------------------------- |
| result | string | Current gas cost per byte of pubdata, scaled by 1e18 (hex) |

## Use Cases

* Computing the pubdata component of transaction fees
* Building precise zkSync-aware fee oracles
* Backend fee budgeting for high-frequency apps
...
const result = await provider.send('zks_gasPerPubdata', []);
console.log(result);
```
{% endtab %}
...
result = zk_web3.zksync._zks_endpoints if 'zks_gasPerPubdata'.startswith('zks_') else None
# Or call any method generically:
result = zk_web3.provider.make_request('zks_gasPerPubdata', [])
print(result)
```
{% endtab %}
{% endtabs %}
