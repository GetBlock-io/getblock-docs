...
| ------ | --------------- | ---------------------------------------------------------------------------- |
| result | array of string | Array `[firstBlock, lastBlock]` in hex — the L2 block range in this L1 batch |

## Use Cases

* Mapping L2 blocks to their containing L1 batch
* Batch-level explorers and analytics
* Replaying batch contents during proof verification
...
const result = await provider.send('zks_getL1BatchBlockRange', [468355]);
console.log(result);
```
{% endtab %}
...
result = zk_web3.zksync._zks_endpoints if 'zks_getL1BatchBlockRange'.startswith('zks_') else None
# Or call any method generically:
result = zk_web3.provider.make_request('zks_getL1BatchBlockRange', [468355])
print(result)
```
{% endtab %}
{% endtabs %}
