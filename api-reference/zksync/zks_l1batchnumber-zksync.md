...
| ------ | ------ | ----------------------------- |
| result | string | Current L1 batch number (hex) |

## Use Cases

* Batch-level indexing and analytics
* Tracking L1 settlement progress
* Monitoring zkSync's L1 commit cadence
...
const result = await provider.send('zks_L1BatchNumber', []);
console.log(result);
```
{% endtab %}
...
result = zk_web3.zksync._zks_endpoints if 'zks_L1BatchNumber'.startswith('zks_') else None
# Or call any method generically:
result = zk_web3.provider.make_request('zks_L1BatchNumber', [])
print(result)
```
{% endtab %}
{% endtabs %}
