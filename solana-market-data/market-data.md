...
{% hint style="warning" %}
`hydrate` is not only an opening snapshot. It sets the size of the row set the subscription maintains: the service sends that many rows up front and then holds the set at that size, evicting the oldest rows as new ones arrive. Size it to the depth of history you actually want to keep.
{% endhint %}
